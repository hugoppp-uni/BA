...
In contract, the Depth API provides a single depth image, where each pixel has a depth value.
...
The tree is used to sparsely partition three-dimensional space into smaller regions and allows for efficient traversal in logarithmic time.
...
There are generally two approaches to utilize an octree for point cloud storage.

One approach is to store the points and their data in the leaf nodes of the octree.
Each node of the octree saves the coordinate of the point and its confidence value.
...
A different approach is to use an octree with a fixed depth, where each leaf represents a region in space.
The center coordinate of the leaf node can be inferred as the point in space, thus saving the coordinates of the point explicitly is not required.
To find the coordinates of a given node, the tree needs to be traversed while keeping track of the extent and center coordinate of the current node.
To prevent noise, confidence values can be stored to replace lower confidence values with higher ones, similar to the first approach.
Using the second approach will also result in quantization of the point cloud.
